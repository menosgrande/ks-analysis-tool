...
ローカル動画をブラウザに読み込むだけで、骨格の 2D/3D 描画・関節角度のグラフ表示・軌跡トレース・可動域(ROM)集計・解析データのエクスポートが行える Web アプリ。

- **インストール不要** — `index.html` をブラウザで開くだけ
- **サーバー不要** — すべての処理はクライアントサイドで完結
- **外部依存は CDN のみ** — Three.js / MediaPipe Tasks API / Font Awesome
- **HTML / CSS / JS の 3 ファイル構成**（`index.html` / `style.css` / `app.js`）。すべて同一フォルダに置けば `index.html` のダブルクリック（`file://`）だけで従来通り動作する（`<script src>` と `<link>` は CORS 制限を受けないため）
...
| ROM レポート | 関節ごとの最小・最大・可動域(ROM)・平均角度・発生時刻を集計。全体 / A-B 区間 / 保存済み区間から範囲選択、CSV 出力可 |
| 角速度・角加速度グラフ | 角度グラフを「角度／角速度／角加速度」に切替表示。隣接フレーム差分（有限差分）で算出、AB ジャンプ等の不連続点は自動でスキップ |
| 実寸(cm)キャリブレーション | 動画上の2点をクリックし実測cmを入力 → px/cm 比を保持 |
| 距離計測(cm) | キャリブレーション後、任意の2点間を実寸cmに換算して表示 |
...
データ ▾ → 可動域(ROM)レポート
  対象範囲（全体 / 現在の A-B 区間 / 保存済み区間）を選択すると、
  関節ごとの 最小・最大・ROM・平均・最小/最大発生時刻 をテーブル表示。
  「CSVでダウンロード」で rom_report_xxx.csv として出力可能。

データ ▾ → 実寸(cm)キャリブレーション
  動画上の基準となる2点をクリック → 実際の距離(cm)を入力。
  以後、距離計測ツールで実寸換算ができるようになる。

データ ▾ → 距離を計測(cm)
  キャリブレーション済みの場合、動画上の任意の2点間を
  実寸cmに換算して表示（要事前キャリブレーション）。
```

### 角度グラフの表示切替

```
グラフ右上のタブ「角度 / 角速度 / 角加速度」で切替可能。
角速度・角加速度は隣接フレームの有限差分（dθ/dt, d²θ/dt²）で算出。
AB ジャンプ・シーク直後など時刻が不連続な箇所は自動的に線を切って表示する。
```
...
├── index.html    # HTML 構造のみ（<link>/<script src> で分離ファイルを参照）
├── style.css     # 全スタイル
├── app.js        # 全ロジック（state・描画・エクスポート・ROM レポート等）
├── README.md     # このファイル
...
- **3 ファイル構成**: `index.html` / `style.css` / `app.js` は必ず同一フォルダに置く。1 つだけ移動・共有すると動作しない
- **実寸キャリブレーション**: カメラ距離・角度が変わると再キャリブレーションが必要。正面から撮影していない映像では誤差が大きくなる
- **角速度・角加速度**: 隣接フレーム差分による近似値。フレームレートが低い動画やノイズの多い区間ではEMAスムージング後でも振動しやすい
