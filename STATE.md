...
| A-3（Pose安定性・破綻条件診断） | 診断ツール完成、動画を目視して動作フェーズと対応付ける作業が残り | Part 2 |
| M-1（app.js分割） | 未着手。状態遷移契約の明文化が先 | Part 1 |

**次にやることの基本順序**：UX-2の原因A（バグ、コスト小）→ A-3を閉じる（解釈作業のみ）→ M-1 → 競合調査から出た機能候補の検討

**重要な設計上の警告（A-3の発見）**：`hip_r visibility=100%` でも一定の角度スパイクが出た。
**visibilityが高い＝解析値が安定、ではない**。M-1で状態遷移契約を書く際やReliability表示を
将来検討する際、「visibilityが高ければ安全」という前提で設計しないこと。
...
| UX-2 (Major, 未着手) | モバイル(iPhone)で操作がごちゃごちゃして分かりにくい。原因診断のみ実施、実装は未着手 | **原因A（バグ）**: カメラ位置ボタン(`.viewport-controls`, 正/側/上)が`.view-split`(2D+3D共通の親)を基準に`position:absolute;top:10px;right:10px`で配置されている。PC版は`.view-split{display:flex}`(横並び)のため右上が自然に3D側に来るが、`@media(max-width:700px)`で`.view-split{flex-direction:column}`に変わり2D→3D縦積みになった際、`.viewport-controls`は同じ基準のまま右上固定なので**2D側の右上に乗ってしまう**。修正案：`#three-container`基準の配置に変更（モバイル時のみ）。**原因B（設計課題）**: 関節タップ選択が動画上の`#tap-select-layer`のみに存在し、角度グラフ(`#graph-overlay`)・数値表示(`#angle-labels`)側にはタップ判定がない。「数値が動いているところを直接タップしたい」という要望あり。動画上タップと数値上タップの両方を同じ選択状態に紐付ける実装が必要、UIの当たり判定設計を要するため原因Aより作業量大 |
